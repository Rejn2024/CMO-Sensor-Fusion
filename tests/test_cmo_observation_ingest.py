...
def test_write_observation_emits_phase_two_ontology_cypher():
    ...
    assert "MERGE (emitter_sensor:Sensor" in statement
    ...
    assert "HAS_EMITTER" in statement
    assert "EMITTED_BY" in statement
    assert "DETECTED_BY_PLATFORM" in statement
    assert "MERGE (platform)-[:HAS_SENSOR]->" not in statement
